...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // frames. Handle Pending, NeedsCursor, Fullscreen, and errors as below.
    let _ = status;
    Ok(())
}
```
...
Choose `Mode::Adaptive` only for a TTY destination. It starts in glass mode, then falls back to a clipped guest-sized region relative to the **current host cursor on the normal screen**, never the alternate screen. A cursor in column 1 starts the region on its current row; otherwise it starts on the following row. Only overflowing height is scrolled, and the stored origin is reused for updates, resizes and recovery. `Reversibility::OneWay` keeps that fallback; `Reversible` resumes after sequential row advancement and new text (or clear and text), confirmed for 100 ms with the cursor at the text frontier. It continues at the retained cursor without clearing or replaying the region; no `CLS` is required. Idle frames or same-line updates alone cannot trigger recovery. A non-TTY adaptive destination is an error, not an automatic switch to glass.

Handle `Status::Pending` by continuing normal vsync submissions. Unrepresentable text must persist for 100 ms: 5/6/7 elapsed intervals at 50/60/70 Hz after the first bad snapshot (6/7/8 bad observations total). Recovery cancels confirmation; cursor-up alone is not failure. Confirmed failure yields `Error::Unrepresentable` in pure glass. Adaptive mode returns `Status::NeedsCursor` (C `TIGT_PRESENTER_NEEDS_CURSOR`, value 4) without writing if a fresh host position is needed; only `Status::Fullscreen` means fallback is active. Pure-glass failure is sticky until `reset`; the consumer decides recovery and must prepare its destination before resetting. The 70 Hz option reserves future cadence only, not VGA support.

On `NeedsCursor`, the consumer obtains the actual host cursor using DSR (`ESC [ 6 n`), demultiplexes the terminal's `ESC [ row ; column R` reply from user input, and calls `presenter.observe_cursor(column, row)` / `tigt_presenter_observe_cursor` before resubmitting at the next guest vsync. These are **one-based host coordinates** after preceding output, not the zero-based guest cursor. The observation call reads and writes nothing; tigt tracks its subsequent output. Call `forget_cursor()` / `tigt_presenter_forget_cursor` after external output or resume. Reset and local-echo registration also invalidate the observation. The library remains output-only: the consumer owns DSR/input handling and terminal-mode changes.

The [presenter reference](reference.md#glass-tty-and-adaptive-presentation) specifies exact BS/CR/NL/FF, overprinting, scrolling and clear rules. Use zero `hints` for ordinary output. For hardware-disabled output, set `TIGT_PRESENT_VIDEO_DISABLED` / `presenter::VIDEO_DISABLED`; the prior image is held for 200 ms (first 9 frames at 50 Hz or 11 at 60 Hz). Set `TIGT_PRESENT_VIDEO_MEMORY_CHANGED` / `presenter::VIDEO_MEMORY_CHANGED` whenever any raw VRAM byte differs from the last submitted snapshot, including on the initial snapshot. That immediately releases the hold for the rest of the disable interval, even if the decoded blank cells did not change. Reenable is immediate and enabled `CLS` is never held. `Pending` retains the current input mode, including during a full-screen hold. Vertical tabs and ISO-8859-1 remain deferred.

For host cooked input with echo, register each **already-displayed finalized line** through `presenter.local_echo(&decoded_line)` / `tigt_presenter_local_echo` before delivering any of its keys to the guest. Do this only when input and output refer to the same echoing TTY, never for pure-pipe input. Keep host canonical editing and echo enabled; pass the final decoded single-cell Unicode scalars with cooked edits folded, including TAB and the already-displayed LF, not raw editing keystrokes. The call copies the data and emits nothing. The bounded capacity is `presenter::LOCAL_ECHO_MAX` / `TIGT_PRESENTER_LOCAL_ECHO_MAX` (4096); invalid input returns `Argument`, overflow returns `Unrepresentable`.

Local echo records actual host output, unlike speculative `notify` below. Confirmed matching guest glyphs and newlines consume the accounting without being printed again; the final input trajectory accounts for automatic guest soft wraps without extra newlines. Mismatch fails representability rather than replaying the echoed line. If the consumer switches to raw input for active fallback, preserve unread cooked text and restore cooked editing when reversible presentation returns to glass. See [local-echo accounting](reference.md#already-displayed-local-echo).
