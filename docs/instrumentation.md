...
iTerm2 inline images carry a 320×200 presentation PNG: 160-wide logical pixels are duplicated, 320-wide pixels are unchanged, and 640-wide pairs are averaged per RGB channel, rounded up. Compare the decoded PNG against this normalization, not a lossless native dump; snapshots retain their original dimensions and RGB. A terminal screenshot additionally includes terminal interpolation and display scaling. Unchanged bitmap submissions emit no further images.
